...
class HierarchicalModel(nn.Module):
    ...
    def forward(self, input_data):
        ...
        sentence_embeddings = torch.zeros(
            batch_size, max_sent_count, self.tf1.config.hidden_size
        ).to(input_ids.device)
        ...
        for batch_idx in range(batch_size):
            ...
            if local_sent_count == 0:
                ...
                local_sent_count = 1
                sentence_embeddings[batch_idx, :local_sent_count] = output1[batch_idx][
                    0
                ]
            else:
                ...
                if self.sentence_pooling != "none":
                    ...
                    for sent_indx in range(local_sent_count):
                        ...
                        if self.sentence_pooling == "sum":
                            sentence_embeddings[batch_idx, sent_indx] = torch.sum(
                                output1[batch_idx][start_index : end_index + 1], axis=0
                            )
                        elif self.sentence_pooling == "mean":
                            sentence_embeddings[batch_idx, sent_indx] = torch.mean(
                                output1[batch_idx][start_index : end_index + 1], axis=0
                            )
                        elif self.sentence_pooling == "max":
                            sentence_embeddings[batch_idx, sent_indx], _ = torch.max(
                                output1[batch_idx][start_index : end_index + 1], axis=0
                            )
                        elif self.sentence_pooling == "min":
                            sentence_embeddings[batch_idx, sent_indx], _ = torch.min(
                                output1[batch_idx][start_index : end_index + 1], axis=0
                            )
                    ...
                    sentence_embeddings[batch_idx, :local_sent_count] = output1[
                        batch_idx
                    ][end_token_lookup[batch_idx]]
        ...
        # get last layer sequence output
        output2 = self.tf2(
            inputs_embeds=sentence_embeddings, attention_mask=sentence_attention_mask
        )[0]
