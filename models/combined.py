...
class CombinedModel(nn.Module):
    def __init__(self, args, freeze_emb_layer=True):
        ...
        self.tf1 = AutoModel.from_pretrained(
            args.model_name,
        ...
        if args.use_pretrained_tf2:
            ...
            self.tf2 = AutoModel.from_config(
        ...
        if self.sentence_pooling == "attention":
            self.linear_weight = nn.Linear(
                self.tf1.config.hidden_size, self.tf1.config.hidden_size
            )
            self.linear_value = nn.Parameter(
                nn.init.xavier_uniform_(
                    torch.FloatTensor(self.tf1.config.hidden_size, 1).type(
    ...
    def forward(self, input_data):
        ...
        sent_fact_embeddings = torch.zeros(
            batch_size, max_sent_fact_seq, self.tf1.config.hidden_size
        ).to(input_ids.device)
        sent_fact_attention_mask = torch.zeros(
            batch_size, max_sent_fact_seq, dtype=torch.long
        ).to(input_ids.device)
        ...
        for batch_idx in range(batch_size):
            ...
            if local_sent_count == 0:
                ...
                local_sent_count = 1
                sent_fact_embeddings[batch_idx, :local_sent_count] = output1[batch_idx][
                    0
                ]
            else:
                # define different type of sub-word pooling for sentence embedding
                if self.sentence_pooling != "none":
                    with torch.no_grad():
                        # get individual sentence offset
                        sentences_offsets = self._get_sentences_offsets(
                            end_token_lookup[batch_idx]
                        )

                    for sent_indx in range(local_sent_count):
                        start_index, end_index = sentences_offsets[sent_indx]
                        if self.sentence_pooling == "sum":
                            sent_fact_embeddings[batch_idx, sent_indx] = torch.sum(
                                output1[batch_idx][start_index : end_index + 1], axis=0
                            )
                        elif self.sentence_pooling == "mean":
                            sent_fact_embeddings[batch_idx, sent_indx] = torch.mean(
                                output1[batch_idx][start_index : end_index + 1], axis=0
                            )
                        elif self.sentence_pooling == "max":
                            sent_fact_embeddings[batch_idx, sent_indx], _ = torch.max(
                                output1[batch_idx][start_index : end_index + 1], axis=0
                            )
                        elif self.sentence_pooling == "min":
                            sent_fact_embeddings[batch_idx, sent_indx], _ = torch.min(
                                output1[batch_idx][start_index : end_index + 1], axis=0
                            )
                        elif self.sentence_pooling == "attention":
                            # transform the sub-word representation
                            subword_rep = torch.tanh(
                                self.linear_weight(
                                    output1[batch_idx][start_index : end_index + 1]
                                )
                            )
                            attention_weights = torch.softmax(
                                subword_rep.mm(self.linear_value), dim=0
                            )
                    ...
                    sent_fact_embeddings[batch_idx, :local_sent_count] = output1[
                        batch_idx
                    ][end_token_lookup[batch_idx]]

            # processing factual information
            local_facts_count = facts_count[batch_idx].item()
            facts_outputs = self.tf2(
                input_ids=facts_ids[batch_idx][:local_facts_count],
                attention_mask=facts_mask[batch_idx][:local_facts_count],
            )[0]
            # retrieving the [CLS] token for fact extracted
            sent_fact_embeddings[
                batch_idx, local_sent_count : local_sent_count + local_facts_count
            ] = facts_outputs[:, 0, :]

            # sentence+fact masking for next layer
            sent_fact_attention_mask[
                batch_idx, : local_sent_count + local_facts_count
            ] = 1

        # get last layer sequence output
        output2 = self.tf2(
            inputs_embeds=sent_fact_embeddings, attention_mask=sent_fact_attention_mask
        )[0]
