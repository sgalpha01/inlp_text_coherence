...
from featurize_dataset import featurize_test_dataset, featurize_train_dataset
...
from dataset_processor.featurize_dataset import CombinedFeaturizer, MTLFeaturizer

# now we can import the all modules present in project folder
from logger import LOG_LEVELS, MyLogger
from models import *
from utils.common import full_clean_directory, load_file
    ...
    def forward(self, input_data):
        ...
        if self.config_args.arch in ["mtl", "combined"] and not args.inference:
            ...
            entailment_data_idx, task_specific_data_idx = (
                get_task_specific_dataset_index(d_ids)
            )
...
class EssayScoringModel(nn.Module):
    def __init__(self, args):
        ...
        if self.config_args.enable_coherence_signal > 0:
            ...
            hidden_size += 768
        self.task_head = EssayScorer(
            hidden_size, args.dropout_rate, bias=prompt_mean[args.prompt_id]
        )
    ...
    def forward(self, input_data):
        if self.config_args.enable_coherence_signal > 0:
            ...
            essay_rep = torch.cat(
                [self.essay_encoder(input_data[:-1]), coherence_rep], dim=-1
            )
        else:
...
def store_text_coherence_vectors(args, fold_id, model, iterator):
    ...
    assert len(global_essay_id) == len(
        global_doc_rep
    ), "mismatch between essay id and doc representation"
    final_res = {}
    ...
    save_filename = os.path.join(
        ...
        "%s-coherence-vector-%d.json" % (args.arch, fold_id),
    )
    with open(save_filename, "w") as dfile:
        json.dump(final_res, dfile)
    args.logger.info(
        "successfully stored the coherence vector to location : %s" % save_filename
    )
    # upload coherence vector file to wandb
    wandb.save(save_filename)


def evaluate_sentence_ordering_model(
    args, fold_id, model, iterator, set_type="val", inference=False
):
    # set the model in eval mode so the dropout and other training parameter will not be effective
    model.eval()
    batch_loss = []
    ...
    for batch in iterator:
            ...
            if len(entailment_data_idx):
                ...
                with torch.no_grad():
                    ...
                    te_acc = accuracy_score(
                        entailment_label_ids.view(-1).tolist(),
                        te_pred.softmax(dim=-1).view(-1, 2).max(dim=-1)[1].tolist(),
                    )
                ...
                online_logger_data.update(
                    {
                        "batch_%s_te_loss_%d" % (set_type, fold_id): te_loss.item(),
                        "batch_%s_te_acc_%d" % (set_type, fold_id): te_acc,
                    }
                )
                loss += te_loss
            online_logger_data.update(
                {
                    "te_%s_data_count_%d"
                    % (set_type, fold_id): len(entailment_data_idx),
                    "ranking_%s_data_count_%d"
                    % (set_type, fold_id): len(task_specific_data_idx),
                }
            )
        ...
        if len(task_specific_data_idx):
            ...
            with torch.no_grad():
                ...
                task_loss = F.margin_ranking_loss(
                    phi_a, phi_b, label_ids, margin=args.margin
                )
        ...
        with torch.no_grad():
            ...
            if args.arch in ["mtl", "combined"] and not inference:
                online_logger_data.update(
                    {"batch_%s_ranking_loss_%d" % (set_type, fold_id): task_loss}
                )

            online_logger_data.update(
                {"batch_%s_combined_loss_%d" % (set_type, fold_id): loss}
            )
    ...
    args.logger.info(
        " %s_loss: %f, %s_accuracy: %f" % (set_type, avg_loss, set_type, pra)
    )
    ...
    if (
        args.arch == "mtl" or (args.arch == "combined" and args.disable_mtl <= 0)
    ) and not inference:
        ...
        args.logger.info(
            " %s_ranking_loss: %f, %s_te_loss:%f, %s_te_accuracy: %f"
            % (set_type, avg_task_loss, set_type, avg_te_loss, set_type, te_accuracy)
        )
...
def sentence_ordering_training_loop(
    args, fold_id, model, train_iterator, valid_iterator
):
    ...
    optimizer = transformers.AdamW(
        optimizer_grouped_parameters, lr=args.rank_learning_rate
    )
    ...
    for epoch in range(1, args.rank_epochs + 1):
        args.logger.info("==" * 30)
        args.logger.info(
            "[Train sentence ordering] Epoch [fold - %d / %d] : %d / %d"
            % (fold_id, args.kfolds, epoch, args.rank_epochs)
        )
        batch_loss = []
        batch_task_loss = []
        ...
        for batch in pbar:
            online_logger_data = {}
            input_data, label_ids = [x.to(device) for x in batch[:-1]], batch[-1].to(
                device
            )
            # dummy indicator (when MTL is inactive) variable for task specific data instances
            task_specific_data_idx = [i for i in range(label_ids.view(-1).size(0))]
            ...
            if args.arch in ["mtl", "combined"]:
                ...
                if len(entailment_data_idx):
                    ...
                    online_logger_data.update(
                        {
                            "batch_train_te_loss_%d" % fold_id: te_loss.item(),
                            "batch_train_te_acc_%d" % fold_id: te_acc,
                        }
                    )
                    loss += te_loss
                online_logger_data.update(
                    {
                        "te_data_count_%d" % fold_id: len(entailment_data_idx),
            ...
            with torch.no_grad():
                ...
                if args.arch in ["mtl", "combined"]:
                    online_logger_data.update(
                        {"batch_train_ranking_loss_%d" % fold_id: task_loss}
                    )

                online_logger_data.update(
                    {"batch_train_combined_loss_%d" % fold_id: loss}
                )
                wandb.log(online_logger_data)
        ...
        if args.arch == "mtl" or (args.arch == "combined" and args.disable_mtl <= 0):
            ...
            te_accuracy = accuracy_score(all_te_actual_label, all_te_pred)
            args.logger.info(
                " train_ranking_loss: %f, train_te_loss:%f, train_te_accuracy: %f"
                % (avg_task_loss, avg_te_loss, te_accuracy)
            )
            epoch_end_logger_data.update(
                {
        ...
        # model evaluation
        with torch.no_grad():
            _, val_pra = evaluate_sentence_ordering_model(
                args, fold_id, model, valid_iterator
            )
            if max_val_pra <= val_pra:
                args.logger.info(
                    "[validation set] new best val accuracy: %f, previous best: %f"
                    % (val_pra, max_val_pra)
                )
                ...
                with open(
                    os.path.join(
                        ...
                        "%s-%s-ranking-epoch-%d.pt"
                        % (args.arch, args.model_name.replace("/", "-"), epoch),
                    ),
                    "wb",
                ) as model_cache_path:
...
def evaluate_model(args, fold_id, model, iterator, set_type="val", inference=False):
    ...
    for batch in iterator:
        online_logger_data = {}
        prompt_ids, input_data, label_ids = (
            batch[0].to(device),
            [x.to(device) for x in batch[1:-1]],
            batch[-1].to(device),
        )
        loss = torch.tensor(0.0).to(device)
        task_loss = torch.tensor(0.0).to(device)
        ...
        with torch.no_grad():
            ...
            if args.arch in ["mtl", "combined"] and not inference:
                online_logger_data.update(
                    {"batch_%s_scoring_loss_%d" % (set_type, fold_id): task_loss}
                )

            online_logger_data.update(
                {"batch_%s_scoring_loss_%d" % (set_type, fold_id): loss}
            )
            wandb.log(online_logger_data)
    ...
    args.logger.info(
        " %s_scoring_loss: %f, %s_weighted kappa score: %f"
        % (set_type, avg_loss, set_type, wks)
    )
...
def training_loop(args, fold_id, model, train_iterator, valid_iterator):
    ...
    optimizer = transformers.AdamW(
        optimizer_grouped_parameters, lr=args.score_learning_rate
    )
    ...
    for epoch in range(1, args.score_epochs + 1):
        args.logger.info("==" * 30)
        args.logger.info(
            "[Train essay scoring] Epoch [fold - %d / %d] : %d / %d"
            % (fold_id, args.kfolds, epoch, args.score_epochs)
        )
        batch_loss = []
        batch_task_loss = []
        ...
        for batch in pbar:
            ...
            with torch.no_grad():
                ...
                pbar.set_description("loss: %f" % (loss.item()))
                online_logger_data.update(
                    {"batch_train_scoring_loss_%d" % fold_id: loss}
                )
                wandb.log(online_logger_data)
        ...
        with torch.no_grad():
            ...
            if max_kappa_score <= val_kappa_score:
                args.logger.info(
                    "[validation set] new best weighted kappa score: %f, previous best: %f"
                    % (val_kappa_score, max_kappa_score)
                )
                full_clean_directory(args.model_save_path)
                with open(
                    os.path.join(
...
def filter_dataset(prompt_id, dataset):
    return (
        dataset
        if prompt_id == 0
        else [x for x in dataset if x["essay_set"] == prompt_id]
    )
...
def load_checkpoint(model, args, sentence_ordering=False):
    ...
    if sentence_ordering:
        prefix_str = "%s-%s-ranking-epoch-" % (
            args.arch,
            args.model_name.replace("/", "-"),
        )

    args.logger.info(
        "loading the %s checkpoint ..."
        % ("sentence ranking" if sentence_ordering else "sentence scoring")
    )
    file_list = [
        x for x in os.listdir(args.model_save_path) if x.startswith(prefix_str)
    ]

    assert len(file_list) > 0, "no checkpoint exists"
    if len(file_list) != 1:
        args.logger.critical(
            "found %d files in checkpoint dir %s"
            % (len(file_list), args.model_save_path)
        )
...
def init(args):
    ...
    # loading the dataset
    raw_dataset = load_file(
        os.path.join(args.processed_dataset_path, "AES", "train.jsonl")
    )
    ...
    if args.arch in ["mtl", "combined"]:
        ...
        args.corpus = "asap"
        wrapper_featurizer = (
            MTLFeaturizer(args) if args.arch == "mtl" else CombinedFeaturizer(args)
        )
    ...
    for fold_count in range(args.kfolds):
        ...
        args.logger.info(
            "dataset count | train : %d [%.2f], dev : %d [%.2f] and test : %d [%.2f]"
            % (
                len(train_index_data),
                len(train_index_data) / len(filtered_dataset),
                len(dev_index_data),
                len(dev_index_data) / len(filtered_dataset),
                len(test_index_data),
                len(test_index_data) / len(filtered_dataset),
            )
        )
        if args.enable_coherence_signal > 0 and args.coherence_vector:
            args.logger.info(
                "using the pre-calculated coherence vectors : %s"
                % os.path.abspath(args.coherence_vector)
            )
        if args.enable_coherence_signal > 0 and args.coherence_vector is None:
            # getting sentence ranking dataset
            featurize_train_dataset(
                args, "train", train_index_data, fold_index, sentence_ordering=True
            )
            featurize_train_dataset(
                args, "dev", dev_index_data, fold_index, sentence_ordering=True
            )
            ...
            train_iterator = get_dataset_loaders(
                os.path.join(
                    args.processed_dataset_path, "featurized_dataset", "train.jsonl"
                ),
                batch_size=args.rank_batch_size,
                float_label=False,
            )
            valid_iterator = get_dataset_loaders(
                os.path.join(
                    args.processed_dataset_path, "featurized_dataset", "dev.jsonl"
                ),
                batch_size=args.rank_batch_size,
                float_label=False,
            )
            ...
            args.logger.info(
                "Ranking model has %d trainable parameters" % (count_parameters(model))
            )
            args.logger.info(model)

            sentence_ordering_training_loop(
                args, fold_index, model, train_iterator, valid_iterator
            )

            # store textual coherence vector for the each essay for a selected prompt
            featurize_test_dataset(
                args, test_index_data, fold_index, sentence_ordering=True
            )
            test_iterator = get_dataset_loaders(
                os.path.join(
                    args.processed_dataset_path, "featurized_dataset", "test.jsonl"
                ),
                batch_size=args.rank_batch_size,
            )
            with torch.no_grad():
        ...
        scoring_train_iterator = get_dataset_loaders(
            os.path.join(
                args.processed_dataset_path, "featurized_dataset", "train.jsonl"
            ),
            batch_size=args.score_batch_size,
        )
        scoring_valid_iterator = get_dataset_loaders(
            os.path.join(
                args.processed_dataset_path, "featurized_dataset", "dev.jsonl"
            ),
            batch_size=args.score_batch_size,
        ...
        args.logger.info(
            "scoring model has %d trainable parameters" % (count_parameters(model))
        )
        args.logger.info(model)

        training_loop(
            args, fold_index, model, scoring_train_iterator, scoring_valid_iterator
        )
        ...
        with torch.no_grad():
            ...
            _, test_wks = evaluate_model(
                args, fold_index, model, test_iterator, set_type="test", inference=True
            )
    ...
    args.logger.info(
        ">>>> %d-fold quadratic kappa score : %f [%f]"
        % (args.kfolds, np.mean(test_kappa_score_list), np.std(test_kappa_score_list))
    )
    args.logger.info("kappa scores : %s" % test_kappa_score_list)
    wandb.log(
        {
            "%d-fold_mean_kappa_score" % args.kfolds: np.mean(test_kappa_score_list),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enable_coherence_signal",
        default=1,
        type=int,
        help="train the sentence ranker if it set to greater than 0",
    )
    parser.add_argument(
        "--coherence_vector", type=str, help="specify the coherence vector file."
    )
    parser.add_argument(
        "--score_epochs",
        default=10,
    ...
    parser.add_argument(
        "--kfolds", type=int, default=5, help="specify number of folds to use."
    )
    parser.add_argument(
        "--prompt_id",
        type=int,
    ...
    logger_exp_name = (
        "%s-wsj-v2-%s-prompt-%d" % (full_arch_name, args.model_name, args.prompt_id)
    ).replace("/", "-")
    args.logger = MyLogger(
        "",
        os.path.join(currentdir, "%s.log" % logger_exp_name),
        use_stdout=True,
