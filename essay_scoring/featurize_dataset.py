...
def _featurize_sentence_ordering_dataset(args, set_type, dataset):
    ...
    mtl_with_fact_aware_model = (
        args.arch == "mtl" and args.mtl_base_arch == "fact-aware"
    )
    mtl_with_hierarchical_model = (
        args.arch == "mtl" and args.mtl_base_arch == "hierarchical"
    )
    ...
    args.logger.info(
        "featurizing the essay ranking corpus for task with model architecture: %s"
        % (args.arch)
    )
    ...
    for i in range(total_count - 1):
        ...
        for j in range(sample_count):
            ...
            doc1_essay_score = get_normalized_score(
                doc1["score"], low_score, high_score
            )
            doc2_essay_score = get_normalized_score(
                doc2["score"], low_score, high_score
            )
            ...
            if args.arch in ["hierarchical", "combined"] or mtl_with_hierarchical_model:
                temp["doc_a"] = sent_sep.join(
                    [" ".join(x) for x in temp["doc_a_sentences"]]
                )
                temp["doc_b"] = sent_sep.join(
                    [" ".join(x) for x in temp["doc_b_sentences"]]
                )
    ...
    if include_facts:
        args.logger.debug(
            "<<INTEGRATING>> factual information to essay evaluation dataset"
        )
        args.logger.debug(
        ...
        for data_instance in tqdm(data_list):
            (
                doc_a_processed_sents,
                doc_a_processed_facts,
                doc_a_is_truncated,
                doc_a_is_zero_fact,
            ) = process_sentences_and_facts(
                data_instance["doc_a_sentences"],
                data_instance["doc_a_facts"],
                args.max_seq_len,
            )
            (
                doc_b_processed_sents,
                doc_b_processed_facts,
                doc_b_is_truncated,
                doc_b_is_zero_fact,
            ) = process_sentences_and_facts(
                data_instance["doc_b_sentences"],
                data_instance["doc_b_facts"],
                args.max_seq_len,
            )
            ...
            data_instance["doc_a"] = sent_sep.join(
                [" ".join(x) for x in doc_a_processed_sents]
            )
            data_instance["doc_b"] = sent_sep.join(
                [" ".join(x) for x in doc_b_processed_sents]
            )
        ...
        args.logger.debug(
            "datapoints truncated | doc_a : %d, doc_b : %d (maximum seq length : %d)"
            % (doc_a_truncated, doc_b_truncated, args.max_seq_len)
        )
        args.logger.debug(
            "datapoints contains no facts | doc_a : %d, doc_b : %d out of %d document pair"
            % (doc_a_zero_fact, doc_b_zero_fact, len(data_list))
        )
        args.logger.debug("--" * 30)

    final_data = featurize_data(
        tokenizer,
        fact_tokenizer,
        include_facts,
        args.max_fact_count,
        args.max_seq_len,
        args.max_fact_seq_len,
        data_list,
        args.logger,
    )
    assert len(final_data) == len(
        data_list
    ), "essay_scoring:featurize_dataset > datainstance count mismatch between featurized and actual dataset"
    # label distribution
    args.logger.info(
        "label distribution in %s essay ranking dataset (total count: %d)"
        % (set_type, len(final_data))
    )
...
def _featurize_sentence_ordering_inference_data(args, set_type, dataset):
    ...
    args.logger.info(
        "featurizing the inference essay ranking corpus for task with model architecture: %s"
        % (args.arch)
    )
    ...
    if include_facts:
        ...
        for data_instance in tqdm(data_list):
            ...
            data_instance["doc_a_facts"] = doc_a_fact_list
        args.logger.debug(
            "datapoints truncated | doc_a : %d (maximum seq length : %d)"
            % (doc_a_truncated, args.max_seq_len)
        )
        args.logger.debug(
            "datapoints contains no facts | doc_a : %d out of %d document pair"
            % (doc_a_zero_fact, len(data_list))
        )
        args.logger.debug("--" * 30)
...
def _featurize_dataset(args, set_type, dataset, fold_id):
    ...
    data_list = []
    args.logger.info(
        "featurizing the essay scoring corpus for task with model architecture: vanilla"
    )
    low_score, high_score = asap_score_ranges.get(args.prompt_id)
