...
def get_permutated_sentence_pairs(
    data_list,
    include_facts,
    permutations,
    logger,
    ...
    for data in tqdm(data_list):
        ...
        if include_facts:
            ...
            assert len(sentences) == len(
                p_facts
            ), "mismatch in count of sentences and facts"

        if with_replacement:
            permutation_ordering = get_permutations_with_replacement(
                len(sentences), permutations
            )
        else:
        ...
        if inverse:
            permutation_ordering = [
                [len(sentences) - 1 - j for j in range(len(sentences))]
            ]
        ...
        for idx, perm_order in enumerate(permutation_ordering):
            doc_2 = sentence_separator.join(
                [" ".join(sentences[i]) for i in perm_order]
            )
            if include_facts:
    ...
    logger.info(
        "single sentence doc count: %d, total doc count: %d, obtained permutation dataset count: %d"
        % (one_sent_doc_count, total_count, len(res))
    )
    return res
...
def process_sentences_and_facts(sents, facts, max_sequence_length):
    ...
    assert len(sents) == len(
        facts
    ), "mismatch in number of sentences: %d and facts: %d." % (len(sents), len(facts))
    doc_fact_count = 0
...
def featurize_data(
    ...
    for doc in tqdm(dataset):
        ...
        if include_facts:
            ...
            concatenate_all_facts.extend(
                [fact_tokenizer.sep_token.join(x) for x in doc_a_facts]
            )
            ...
            if "doc_b_facts" in doc:
                ...
                concatenate_all_facts.extend(
                    [fact_tokenizer.sep_token.join(x) for x in doc_b_facts]
                )

            max_fact_count_per_doc = max(
                max_fact_count_per_doc, len(doc_a_facts), len(doc_b_facts)
            )
    ...
    logger.info(
        "data preprocessed: %d, sentences preprocessed: %d"
        % (len(position_data), len(concatenate_all_sentences))
    )

    if include_facts:
        logger.info(
            "fact preprocessed: %d, max_fact_count_per_doc: %d"
            % (len(concatenate_all_facts), max_fact_count_per_doc)
        )

    sentences_input_ids, sentences_attention_ids = encode_batch(
        tokenizer, concatenate_all_sentences, max_seq_len
    )
    if include_facts:
        facts_input_ids, facts_attention_ids = encode_batch(
            fact_tokenizer, concatenate_all_facts, max_fact_seq_len
        )
    ...
    for pos in tqdm(position_data):
        ...
        if include_facts:
            ...
            temp["doc_a_facts"] = np.zeros(
                (max_fact_count_per_doc, max_fact_seq_len)
            ).tolist()
            temp["doc_a_facts"][:fact_count] = facts_input_ids[
                start : start + fact_count
            ]
            temp["doc_a_facts_mask"] = np.zeros(
                (max_fact_count_per_doc, max_fact_seq_len)
            ).tolist()
            temp["doc_a_facts_mask"][:fact_count] = facts_attention_ids[
                start : start + fact_count
            ]
            ...
            if "doc_b_fact_offset" in pos and "doc_b_fact_count" in pos:
                ...
                temp["doc_b_facts"] = np.zeros(
                    (max_fact_count_per_doc, max_fact_seq_len)
                ).tolist()
                temp["doc_b_facts"][:fact_count] = facts_input_ids[
                    start : start + fact_count
                ]
                temp["doc_b_facts_mask"] = np.zeros(
                    (max_fact_count_per_doc, max_fact_seq_len)
                ).tolist()
                temp["doc_b_facts_mask"][:fact_count] = facts_attention_ids[
                    start : start + fact_count
                ]
    ...
    logger.debug(
        "%d data instance processed. max sent_seq_length: %d"
        % (len(position_data), max_sent_seq_len)
    )
    if include_facts:
