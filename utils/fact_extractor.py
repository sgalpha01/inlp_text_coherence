...
os.environ["CLASSPATH"] = (
    "/home/tushar/research/source_codes/miniepy/target/minie-0.0.1-SNAPSHOT.jar"
)
...
class MinIEFactExtractor:
    def __init__(self):
        if "CLASSPATH" not in os.environ or "JAVA_HOME" not in os.environ:
            raise Exception(
                "'CLASSPATH' and 'JAVA_HOME' system environment variable not set. Please refer: https://github.com/mmxgn/miniepy"
            )
        self._initialize_core_services()

    def _initialize_core_services(self):
        self.CoreNLPUtils = autoclass("de.uni_mannheim.utils.coreNLP.CoreNLPUtils")
        self.AnnotatedProposition = autoclass(
    ...
    def get_triples(self, sentence):
        ...
        for ap in minie.getPropositions().elements():
            if ap is not None:
                triples.append(
                    [x[1:-1] for x in ap.getTripleAsString().strip().split("\t")]
                )
        return triples
