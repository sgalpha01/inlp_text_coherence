...
from utils.common import (
    load_file,
    store_results,
    normalize_gcdc_sub_corpus,
    encode_batch,
    linearize_facts,
    clean_directory,
)
from utils.data_specific import (
    process_sentences_and_facts,
    featurize_dataset,
    featurize_data,
    get_permutated_sentence_pairs,
)
...
class GCDCFeaturizer:
    def __init__(self, args):
        ...
        if args.sub_corpus.lower() not in [
            x.lower() for x in dataset_options["gcdc"]["sub_corpus"]
        ]:
            raise Exception("%s is not part of GCDC corpus" % (args.sub_corpus))

        args.logger.info(
            "featurizing the gcdc corpus: %s for task: %s with model architecture: %s"
            % (args.sub_corpus, args.task, args.arch)
        )
    ...
    def get_task_specific_labels(self, data_input_map):
        if self.args.task == "sentence-score-prediction":
            annotator_aggreement = [
                data_input_map["label1"],
                data_input_map["label2"],
                data_input_map["label3"],
            ]
            annotator_aggreement = [
                x for x in map(lambda x: int(x), annotator_aggreement)
            ]
            ...
            labels = (
                int(data_input_map["label"]) - 1
            )  # actual gcdc labels are {1=low, 2=medium, 3=high}, transformed to [0=low, 1=medium, 2=high].
            return labels
        else:
            raise Exception(
                "%s task specific label processing is not defined" % self.args.task
            )

    def load_data(self, inference):
        dataset_map = {"train": [], "dev": [], "test": []}
        ...
        self.args.logger.debug(
            "<<LOADING>> GCDC dataset from directory: %s" % gcdc_dataset_path
        )
    ...
    def featurize_dataset(self, inference=False):
        ...
        mtl_with_fact_aware_model = (
            self.args.arch == "mtl" and self.args.mtl_base_arch == "fact-aware"
        )
        mtl_with_hierarchical_model = (
            self.args.arch == "mtl" and self.args.mtl_base_arch == "hierarchical"
        )
        ...
        include_facts = (
            self.args.arch in ["fact-aware", "combined"]
        ) or mtl_with_fact_aware_model
        sent_sep = (
            tokenizer.sep_token
            if (self.args.arch in ["hierarchical", "combined"])
            or mtl_with_hierarchical_model
            else " "
        )
        ...
        if self.args.task in [
            ...
            for set_type, data_list in dataset_map.items():
                for data_instance in data_list:
                    if (
                        self.args.arch in ["hierarchical", "combined"]
                        or mtl_with_hierarchical_model
                    ):
                        data_instance["doc_a"] = sent_sep.join(
                            [" ".join(x) for x in data_instance["sentences"]]
                        )
                    else:
                        data_instance["doc_a"] = data_instance["text"]
                    data_instance["label"] = self.get_task_specific_labels(
                        data_instance
                    )
        ...
        if include_facts:
            self.args.logger.debug(
                "<<INTEGRATING>> factual information to GCDC dataset"
            )
            for set_type, data_list in dataset_map.items():
                self.args.logger.debug(
                    "working on %s dataset (count: %d)" % (set_type, len(data_list))
                )
                truncated = 0
                zero_fact = 0
                for data_instance in tqdm(data_list):
                    processed_sents, processed_facts, is_truncated, is_zero_fact = (
                        process_sentences_and_facts(
                            data_instance["sentences"],
                            data_instance["facts"],
                    ...
                    if self.args.task == "sentence-ordering":
                        ...
                        data_instance["doc_a"] = sent_sep.join(
                            [" ".join(x) for x in processed_sents]
                        )
                ...
                self.args.logger.debug(
                    "%d datapoints truncated (maximum seq length : %d)"
                    % (truncated, self.args.max_seq_len)
                )
                self.args.logger.debug(
                    "%d / %d datapoints contains no facts" % (zero_fact, len(data_list))
                )
            self.args.logger.debug("--" * 30)
        ...
        if self.args.task == "sentence-ordering":
            for set_type, data_list in dataset_map.items():
                ...
                permutated_pair_data_list = get_permutated_sentence_pairs(
                    data_list,
                    include_facts,
                    self.args.permutation_count,
                    self.args.logger,
                    with_replacement=self.args.with_replacement > 0,
                    sentence_separator=sent_sep,
                    inverse=self.args.inverse_pra > 0,
                )
        ...
        for set_type, data_list in dataset_map.items():
            ...
            featurize_dataset(
                ...
                fact_tokenizer,
                include_facts,
                self.args.max_fact_count,
                self.args.max_seq_len,
                self.args.max_fact_seq_len,
                save_file_path,
                data_list,
                self.args.logger,
            )
        ...
        if self.args.task != "sentence-score-prediction":
            for set_type, data_list in dataset_map.items():
                self.args.logger.info(
                    "label distribution in %s dataset (total count: %d)"
                    % (set_type, len(data_list))
                )
...
class WSJFeaturizer:
    def __init__(self, args):
        ...
        args.logger.info(
            "featurizing the wsj corpus for task: %s with model architecture: %s"
            % (args.task, args.arch)
        )
        # cache the cmdline arguments
        self.args = args
    ...
    def load_data(self, inference):
        ...
        self.args.logger.debug(
            "<<LOADING>> WSJ dataset from directory: %s" % wsj_dataset_path
        )
        for set_type in dataset_map:
            self.args.logger.debug("working on %s dataset " % set_type)
    ...
    def featurize_dataset(self, inference=False):
        ...
        dataset_map = self.load_data(inference)
        include_facts = (
            self.args.arch in ["fact-aware", "combined"] or mtl_with_fact_aware_model
        )
        sent_sep = (
            tokenizer.sep_token
            if (self.args.arch in ["hierarchical", "combined"])
...
class MTLFeaturizer:
    def __init__(self, args, **kwargs):
        args.logger.info(
            "featurizing the textual entailment corpus for task: %s with model architecture: %s"
            % (args.task, args.arch)
        )
        # cache the cmdline arguments
        self.args = args
    ...
    def load_data(self):
        ...
        te_dataset_path = os.path.join(
            self.args.processed_dataset_path, textual_entailment_dataset
        )
        self.args.logger.debug(
            "<<LOADING>> %s dataset from directory: %s"
            % (textual_entailment_dataset, te_dataset_path)
        )
        for set_type in dataset_map:
            self.args.logger.debug("working on %s dataset " % set_type)
            file_name = "%s.jsonl" % (set_type)
    ...
    def featurize_dataset(self, inference=False):
        ...
        sentence_ordering_flag = (
            "doc_b" in mtl_featurized_dataset_map[random.choice(set_types)][0]
        )
        ...
        if not inference:
            ...
            for set_type in te_featurized_dataset_map:
                ...
                if include_facts:
                    prev_max_fact_count = len(
                        mtl_featurized_dataset_map[set_type][0]["doc_a_facts"]
                    )
                    prev_max_fact_seq_len = len(
                        mtl_featurized_dataset_map[set_type][0]["doc_a_facts"][0]
                    )
        ...
        for set_type in mtl_featurized_dataset_map:
            if set_type in te_featurized_dataset_map:
                mtl_featurized_dataset_map[set_type].extend(
                    te_featurized_dataset_map[set_type]
                )
        ...
        for set_type, data_list in te_featurized_dataset_map.items():
            self.args.logger.info(
                "label distribution for %s textual entailment dataset (total count: %d)"
                % (set_type, len(data_list))
            )
